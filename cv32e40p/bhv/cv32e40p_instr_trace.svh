...
class instr_trace_t;
  ...
  function void init(int unsigned cycles, bit [31:0] pc, bit compressed, bit [31:0] instr);
    ...
    casex (instr)
      ...
      //Zba
      INSTR_SH1ADD:    this.printRInstr("sh1add");
      INSTR_SH2ADD:    this.printRInstr("sh2add");
      INSTR_SH3ADD:    this.printRInstr("sh3add");

      //Zbs
      INSTR_BCLR:      this.printRInstr("bclr");
      INSTR_BEXT:      this.printRInstr("bext");
      INSTR_BINV:      this.printRInstr("binv");
      INSTR_BSET:      this.printRInstr("bset");
      INSTR_BCLRI:     this.printZbsIInstr("bclri");
      INSTR_BEXTI:     this.printZbsIInstr("bexti");
      INSTR_BINVI:     this.printZbsIInstr("binvi");
      INSTR_BSETI:     this.printZbsIInstr("bseti");

      //Zbc
      INSTR_CLMUL:     this.printRInstr("clmul");
      INSTR_CLMULH:    this.printRInstr("clmulh");
      INSTR_CLMULR:    this.printRInstr("clmulr");

      //Zbb (register-register)
      INSTR_ZBB_ANDN:  this.printRInstr("andn");
      INSTR_ZBB_ORN:   this.printRInstr("orn");
      INSTR_ZBB_XNOR:  this.printRInstr("xnor");
      INSTR_ZBB_MIN:   this.printRInstr("min");
      INSTR_ZBB_MINU:  this.printRInstr("minu");
      INSTR_ZBB_MAX:   this.printRInstr("max");
      INSTR_ZBB_MAXU:  this.printRInstr("maxu");
      INSTR_ZBB_ROL:   this.printRInstr("rol");
      INSTR_ZBB_ROR:   this.printRInstr("ror");
      INSTR_ZBB_ZEXTH: this.printR1Instr("zext.h");

      //Zbb (register-immediate)
      INSTR_ZBB_CLZ:   this.printR1Instr("clz");
      INSTR_ZBB_CTZ:   this.printR1Instr("ctz");
      INSTR_ZBB_CPOP:  this.printR1Instr("cpop");
      INSTR_ZBB_SEXTB: this.printR1Instr("sext.b");
      INSTR_ZBB_SEXTH: this.printR1Instr("sext.h");
      INSTR_ZBB_RORI:  this.printZbsIInstr("rori");
      INSTR_ZBB_ORCB:  this.printR1Instr("orc.b");
      INSTR_ZBB_REV8:  this.printR1Instr("rev8");

      //Zbkb (register-register)
      INSTR_PACK:      this.printRInstr("pack");
      INSTR_PACKH:     this.printRInstr("packh");
 
      //Zbkb 
      INSTR_BREV8:     this.printR1Instr("brev8");
      INSTR_ZIP:       this.printR1Instr("zip");
      INSTR_UNZIP:     this.printR1Instr("unzip");
 
      //Zbkx
      INSTR_XPERM4:    this.printRInstr("xperm4");
      INSTR_XPERM8:    this.printRInstr("xperm8");
  ...
  function void printZbsIInstr(input string mnemonic);
    begin
      regs_read.push_back('{rs1, rs1_value, 0});
      regs_write.push_back('{rd, 'x, 0});
      // seuls les 5 bits [24:20] sont le shamt réel pour Zbs (funct7 occupe [31:25])
      str = $sformatf("%-16s %s, %s, %0d", mnemonic, regAddrToStr(rd), regAddrToStr(rs1), $unsigned(instr[24:20]));
    end
  endfunction  // printZbsIInstr
