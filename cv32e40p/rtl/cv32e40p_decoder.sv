    ...
    unique case (instr_rdata_i[6:0])
      ...
      OPCODE_OPIMM: begin // Register-Immediate ALU Operations
        ...
        unique case (instr_rdata_i[14:12])
          ...
          else if (ZBS && instr_rdata_i[31:25] == 7'b010_0100)
            alu_operator_o = ALU_ZBS_BCLR;
          else if (ZBS && instr_rdata_i[31:25] == 7'b011_0100)
            alu_operator_o = ALU_ZBS_BINV;
          else if (ZBS && instr_rdata_i[31:25] == 7'b001_0100)
            alu_operator_o = ALU_ZBS_BSET;
          else if (ZBB && instr_rdata_i[31:20] == 12'h600)
            alu_operator_o = ALU_ZBB_CLZ;
          else if (ZBB && instr_rdata_i[31:20] == 12'h601)
            alu_operator_o = ALU_FF1;
          else if (ZBB && instr_rdata_i[31:20] == 12'h602)
            alu_operator_o = ALU_CNT;
          else if (ZBB && instr_rdata_i[31:20] == 12'h604)
            alu_operator_o = ALU_ZBB_SEXTB;
          else if (ZBB && instr_rdata_i[31:20] == 12'h605)
            alu_operator_o = ALU_ZBB_SEXTH;
          else if (ZBKB && instr_rdata_i[31:20] == 12'h08F)
        ...
        3'b101: begin
          if (instr_rdata_i[31:25] == 7'b0)
            ...
            alu_operator_o = ALU_ZBS_BEXT;
          else if (ZBB && instr_rdata_i[31:20] == 12'h287)
            alu_operator_o = ALU_ZBB_ORCB;   // orc.b (Zbb only)
          else if ((ZBB || ZBKB) && instr_rdata_i[31:20] == 12'h698)
            alu_operator_o = ALU_ZBB_REV8;   // rev8
          else if (ZBKB && instr_rdata_i[31:20] == 12'h687)
            alu_operator_o = ALU_BREV8;      // brev8
          else if (ZBKB && instr_rdata_i[31:20] == 12'h08F)
            alu_operator_o = ALU_UNZIP;      // unzip
          else if ((ZBB || ZBKB) && instr_rdata_i[31:25] == 7'b011_0000)
            alu_operator_o = ALU_ROR;        // rori
          else
            illegal_insn_o = 1'b1;
        end
      ...
      OPCODE_OP: begin
        ...
        // PREFIX 00/01
          ...
          unique case ({instr_rdata_i[30:25], instr_rdata_i[14:12]})
            ...
            // Zba extension
            {6'b01_0000, 3'b010}: if (ZBA) alu_operator_o = ALU_SH1ADD; else illegal_insn_o = 1'b1;
            {6'b01_0000, 3'b100}: if (ZBA) alu_operator_o = ALU_SH2ADD; else illegal_insn_o = 1'b1;
            {6'b01_0000, 3'b110}: if (ZBA) alu_operator_o = ALU_SH3ADD; else illegal_insn_o = 1'b1;

            // Zbs extension
            {6'b10_0100, 3'b001}: if (ZBS) alu_operator_o = ALU_ZBS_BCLR; else illegal_insn_o = 1'b1; 
            {6'b10_0100, 3'b101}: if (ZBS) alu_operator_o = ALU_ZBS_BEXT; else illegal_insn_o = 1'b1; 
            {6'b11_0100, 3'b001}: if (ZBS) alu_operator_o = ALU_ZBS_BINV; else illegal_insn_o = 1'b1;
            {6'b01_0100, 3'b001}: if (ZBS) alu_operator_o = ALU_ZBS_BSET; else illegal_insn_o = 1'b1; 

                        // Zbc / Zbkc extension 
            {6'b00_0101, 3'b001}: if (ZBC || ZBKC) alu_operator_o = ALU_CLMUL;  else illegal_insn_o = 1'b1; 
            {6'b00_0101, 3'b011}: if (ZBC || ZBKC) alu_operator_o = ALU_CLMULH; else illegal_insn_o = 1'b1; 
            {6'b00_0101, 3'b010}: if (ZBC)         alu_operator_o = ALU_CLMULR; else illegal_insn_o = 1'b1;

            // Zbb / Zbkb extension 
            {6'b10_0000, 3'b111}: if (ZBB || ZBKB) alu_operator_o = ALU_ZBB_ANDN; else illegal_insn_o = 1'b1; // andn
            {6'b10_0000, 3'b110}: if (ZBB || ZBKB) alu_operator_o = ALU_ZBB_ORN;  else illegal_insn_o = 1'b1; // orn
            {6'b10_0000, 3'b100}: if (ZBB || ZBKB) alu_operator_o = ALU_ZBB_XNOR; else illegal_insn_o = 1'b1; // xnor

            {6'b00_0101, 3'b100}: if (ZBB) alu_operator_o = ALU_MIN;  else illegal_insn_o = 1'b1; // min  (Zbb only)
            {6'b00_0101, 3'b101}: if (ZBB) alu_operator_o = ALU_MINU; else illegal_insn_o = 1'b1; // minu (Zbb only)
            {6'b00_0101, 3'b110}: if (ZBB) alu_operator_o = ALU_MAX;  else illegal_insn_o = 1'b1; // max  (Zbb only)
            {6'b00_0101, 3'b111}: if (ZBB) alu_operator_o = ALU_MAXU; else illegal_insn_o = 1'b1; // maxu (Zbb only)

            {6'b11_0000, 3'b001}: if (ZBB || ZBKB) alu_operator_o = ALU_ZBB_ROL; else illegal_insn_o = 1'b1; // rol
            {6'b11_0000, 3'b101}: if (ZBB || ZBKB) alu_operator_o = ALU_ROR;     else illegal_insn_o = 1'b1; // ror

            {6'b00_0100, 3'b100}: begin // zext.h (Zbb, rs2=x0) / pack (Zbkb, rs2 quelconque)
              if (ZBKB) begin
                alu_operator_o = ALU_PACK;
              end else if (ZBB && instr_rdata_i[24:20] == 5'b0) begin
                alu_operator_o = ALU_ZBB_ZEXTH;
              end else begin
                illegal_insn_o = 1'b1;
              end
            end

            {6'b00_0100, 3'b111}: begin // packh (Zbkb)
              if (ZBKB) alu_operator_o = ALU_PACKH;
              else illegal_insn_o = 1'b1;
            end
            
            {6'b01_0100, 3'b010}: begin // xperm4 (Zbkx)
              if (ZBKX) alu_operator_o = ALU_XPERM4;
              else illegal_insn_o = 1'b1;
            end
            {6'b01_0100, 3'b100}: begin // xperm8 (Zbkx)
              if (ZBKX) alu_operator_o = ALU_XPERM8;
              else illegal_insn_o = 1'b1;
            end
